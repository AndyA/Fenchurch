...
{
  my ( $mq1, $mq2 ) = make_mq_pair($db);

  is $mq1->available, 0, "Queue 1 initially empty";
  is $mq2->available, 0, "Queue 2 initially empty";
  ...
  is $mq1->available, 3, "Three message on queue 1";
  is $mq2->available, 1, "One message on queue 2";
}

{
  my ( $mq1, $mq2 ) = make_mq_pair($db);
  is $mq1->available, 3, "Three messages still on queue 1";
  is $mq2->available, 1, "One message still on queue 2";
  ...
  is $mq1->available, 2, "Two messages still on queue 1";
...
{
  my ( $mq1, $mq2 ) = make_mq_pair($db);
  is $mq1->available, 0, "Queue 1 empty";
  is $mq2->available, 2, "Two messages on queue 2";
...

  {
    ...
    eq_or_diff [@got],
     ...
     "Two messages";
    is $mq2->available, 0, "Queue 2 empty";
  }
...
sub make_mq_pair {
  my $db  = shift;
  my $mq1 = make_mq( $db, 'here', 'there' );
  my $mq2 = make_mq( $db, 'hither', 'yon' );
  return ( $mq1, $mq2 );
}
